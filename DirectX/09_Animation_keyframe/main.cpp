...
HRESULT InitIB()
{
	...
	if (FAILED(g_pd3dDevice->CreateIndexBuffer(24 * sizeof(MYINDEX), 0, D3DFMT_INDEX16, D3DPOOL_DEFAULT,
		&g_pIB, NULL)))
		return E_FAIL;
...
void SetupCamera()
{
	...
	D3DXVECTOR3 vEyePt(0.0f, 10.0f, -50.0f);
...
VOID Animate()
{
	...
	t += 0.005f;
	//차일드의 z 축회전 속도
	D3DXMatrixRotationZ(&g_matRChild, GetTickCount() / 500.0f); //자전
	//숫자  => 둘 사이에 거리
	D3DXMatrixTranslation(&g_matTMChild, 3, 3, 3);

	//------------------------------------------------------

	D3DXMatrixRotationZ(&g_matRChild2, GetTickCount() / 500.0f); //자전
	//숫자  => 둘 사이에 거리
	D3DXMatrixTranslation(&g_matTMChild2, 4, 0, 0);

	//------------------------------------------------------

	D3DXMatrixRotationZ(&g_matRChild3, GetTickCount() / 500.0f); //자전
   //숫자  => 둘 사이에 거리
	D3DXMatrixTranslation(&g_matTMChild3, 15, 0, 0);
}
