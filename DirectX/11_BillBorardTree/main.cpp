...
// 나무 꺼
LPDIRECT3DVERTEXBUFFER9		g_pVBTree = NULL;
LPDIRECT3DINDEXBUFFER9		g_pIBTree = NULL;
LPDIRECT3DTEXTURE9			g_pTexHeightTree = NULL;
LPDIRECT3DTEXTURE9			g_pTexDiffuseTree = NULL;
D3DXMATRIXA16				g_matAniTree;
DWORD						g_cxHeightTree = 0;
DWORD						g_czHeightTree = 0;
...
HRESULT InitD3D(HWND hWnd)
{
	...
	g_pd3dDevice->SetRenderState(D3DRS_CULLMODE, D3DCULL_CCW); // 반시계반향 컬링하겠다
	g_pd3dDevice->SetRenderState(D3DRS_ZENABLE, TRUE); //z버퍼를 씀
...
{
	//나무
	if (FAILED(D3DXCreateTextureFromFile(g_pd3dDevice, "tree.bmp", &g_pTexDiffuseTree)))
		return E_FAIL;

	//텍스쳐맵 초기화, Ex는 옵션을 줘서 설정할수있다, D3DFMT_X8 R8G8B8
...
HRESULT InitVB()
{
	...
	g_pTexHeightTree->GetLevelDesc(0, &ddsd);
	g_cxHeightTree = ddsd.Width;
	g_czHeightTree = ddsd.Height;
	ZFLog::GetInstance()->Log("Texturel Size:[%d,%d]", g_cxHeightTree, g_czHeightTree);
	if (FAILED(g_pd3dDevice->CreateVertexBuffer(ddsd.Width*ddsd.Height * sizeof(CUSTOMVERTEX), 0,
		D3DFVF_CUSTOMVVERTEX, D3DPOOL_DEFAULT, &g_pVBTree, NULL)))
		return E_FAIL;

	g_pTexHeightTree->LockRect(0, &d3drc, NULL, D3DLOCK_READONLY);
	VOID* pVerticesTree;
	if (FAILED(g_pVBTree->Lock(0, g_cxHeightTree * g_czHeightTree * sizeof(CUSTOMVERTEX), (void**)&pVerticesTree, 0)))
		return E_FAIL;

	CUSTOMVERTEX v1;
	CUSTOMVERTEX* pV1 = (CUSTOMVERTEX*)pVerticesTree;
	for (DWORD z = 0; z < g_czHeightTree; z++)
	{
		for (DWORD x = 0; x < g_cxHeightTree; x++)
		{
			v1.p.x = (float)x - g_cxHeightTree / 2.0f;
			v1.p.z = -((float)z - g_czHeightTree / 2.0f);
			v1.p.y = ((float)(*((LPDWORD)d3drc.pBits + x + z * (d3drc.Pitch / 4)) & 0x0000ff)) / 10.0f;
			v1.n.x = v1.p.x;
			v1.n.y = v1.p.y;
			v1.n.z = v1.p.z;
			D3DXVec3Normalize(&v1.n, &v1.n);
			v1.t.x = (float)x / (g_cxHeightTree - 1);
			v1.t.y = (float)z / (g_czHeightTree - 1);
			*pV1++ = v1;
...
HRESULT InitIB()
{
	if (FAILED(g_pd3dDevice->CreateIndexBuffer((g_cxHeightTree - 1) * (g_czHeightTree - 1) * 2 * sizeof(MYINDEX), 0,
		D3DFMT_INDEX16, D3DPOOL_DEFAULT, &g_pIBTree, NULL)))
		return E_FAIL;

	MYINDEX		iTree;
	MYINDEX*	pITree;
	if (FAILED(g_pIBTree->Lock(0, (g_cxHeightTree - 1) * (g_czHeightTree - 1) * 2 * sizeof(MYINDEX), (void**)&pITree, 0)))
		return E_FAIL;

	for (DWORD z = 0; z < g_czHeightTree - 1; z++)
	{
		for (DWORD x = 0; x < g_cxHeightTree - 1; x++)
		{
			iTree._0 = (z*g_cxHeightTree + x);
			iTree._1 = (z*g_cxHeightTree + x + 1);
			iTree._2 = ((z + 1)*g_cxHeightTree + x);
			*pITree++ = iTree;

			iTree._0 = ((z + 1)*g_cxHeightTree + x);
			iTree._1 = (z*g_cxHeightTree + x + 1);
			iTree._2 = ((z + 1)*g_cxHeightTree + x + 1);
			*pITree++ = iTree;
		}
	}
...
void CleanUp()
{
	SAFE_RELEASE(g_pTexHeightTree);
	SAFE_RELEASE(g_pTexDiffuseTree);
	SAFE_RELEASE(g_pIBTree);
	SAFE_RELEASE(g_pVBTree);
...
void DrawMesh(D3DXMATRIXA16* pMat)
{
	...
	g_pd3dDevice->SetStreamSource(0, g_pVBTree, 0, sizeof(CUSTOMVERTEX));
	g_pd3dDevice->SetFVF(D3DFVF_CUSTOMVVERTEX);
	g_pd3dDevice->SetIndices(g_pIB);
	g_pd3dDevice->SetIndices(g_pIBTree);
	g_pd3dDevice->DrawIndexedPrimitive(D3DPT_TRIANGLELIST, 0, 0, g_cxHeight * g_czHeight, 0,
		(g_cxHeight - 1) * (g_czHeight - 1) * 2);
	g_pd3dDevice->DrawIndexedPrimitive(D3DPT_TRIANGLELIST, 0, 0, g_cxHeightTree * g_czHeightTree, 0,
		(g_cxHeightTree - 1) * (g_czHeightTree - 1) * 2);
}
...
void Render()
{
	...
	if (SUCCEEDED(g_pd3dDevice->BeginScene()))
	{
		...
		g_pd3dDevice->SetTexture(0, g_pTexDiffuseTree);
		g_pd3dDevice->SetSamplerState(0, D3DSAMP_MAGFILTER, D3DTEXF_LINEAR);
		g_pd3dDevice->SetTextureStageState(0, D3DTSS_TEXCOORDINDEX, 0);
		g_pd3dDevice->SetTextureStageState(0, D3DTSS_COLOROP, D3DTOP_MODULATE);
		g_pd3dDevice->SetTextureStageState(0, D3DTSS_COLORARG1, D3DTA_TEXTURE);
