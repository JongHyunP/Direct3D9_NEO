...
VS_OUTPUT NormalMapping_Pass_0_Vertex_Shader_vs_main(VS_INPUT Input)
{
   ...
   Output.mPositon = mul(Input.mPosition , gWorldViewProjectionMatrix);
   Output.mUV = Input.mUV;
   
   float4 worldPosition = mul(Input.mPosition , gWorldMatrix);
   float3 lightDir = Output.mPositon.xyz - gWorldLightPosition.xyz;
   Output.mLightDir = normalize(lightDir);
   
   float3 viewDir = normalize(Output.mPositon.xyz - gWorldCameraPosition.xyz);
   Output.mViewDir = viewDir;
   
   float3 worldNormal = mul(Input.mNormal , (float3x3)gWorldMatrix);
   Output.N = normalize(worldNormal);
   
   float3 worldTangent = mul(Input.mTanget , (float3x3)gWorldMatrix);
...
<
   string ResourceName = "..\\..\\..\\..\\..\\Desktop\\CPP_Class\\CPP\\DirectX\\fieldstone_SM.tga";
>;
